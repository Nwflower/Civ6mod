...
'REQUIREMENT_NW_PLOT_ADJACENT_'||TerrainType, 'REQUIREMENT_PLOT_ADJACENT_TERRAIN_TYPE_MATCHES'
FROM Terrains;
INSERT OR IGNORE INTO RequirementArguments (RequirementId, Name, Value) SELECT
'REQUIREMENT_NW_PLOT_ADJACENT_'||TerrainType, 'TerrainType', TerrainType
FROM Terrains UNION SELECT
'REQUIREMENT_NW_PLOT_ADJACENT_'||TerrainType, 'MinRange', 1
FROM Terrains UNION SELECT
'REQUIREMENT_NW_PLOT_ADJACENT_'||TerrainType, 'MaxRange', 1
FROM Terrains;
...
WHERE TraitType = 'TRAIT_LEADER_ANTIQUES_AND_PARKS'
  AND ModifierId IN ('WENMEI_YULINMEILI_TEAMPVP', 'TRAIT_WILD_CARD_FAVOR');
