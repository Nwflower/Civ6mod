...
function BBS_ResourceGenerator.Create(args)
	...
	local instance = {
		...
		iTargetPercentage   = 33;
		iStandardPercentage = 33;
		iLuxuryPercentage   = 21;
...
function BBS_ResourceGenerator:__ScoreLuxuryPlots(iResourceIndex, eContinent)
	...
	for i, plot in ipairs(plots) do
		...
		if (ResourceBuilder.CanHaveResource(pPlot, self.eResourceType[iResourceIndex]) and bIce == false) then
			...
			row.MapIndex = plot;
			row.Score = 500;
			row.Score = row.Score / ((ResourceBuilder.GetAdjacentResourceCount(pPlot) + 1) * 10);
...
function BBS_ResourceGenerator:__GetOtherResources()
	...
	--and self.eResourceClassType[index] ~= "RESOURCECLASS_LEY_LINE"
    for row = 0, self.iResourcesInDB do
		local index  = self.aIndex[row];
		if(self.eResourceClassType[index] ~= nil) then
			if (self.eResourceClassType[index] ~= "RESOURCECLASS_STRATEGIC" and self.eResourceClassType[index] ~= "RESOURCECLASS_LUXURY" and
			 self.eResourceClassType[index] ~= "RESOURCECLASS_ARTIFACT" ) then
				if( self.eResourceClassType[index] == "RESOURCECLASS_LEY_LINE")then
					print("RESOURCECLASS_LEY_LINE self.iFrequency[index]:",self.iFrequency[index]);
					print("RESOURCECLASS_LEY_LINE self.eResourceType[index]:",self.eResourceType[index]);
				end
...
function BBS_ResourceGenerator:__PlaceOtherResources()
    ...
    for i, row in ipairs(self.aResourcePlacementOrder) do
	    ...
	    if(eResourceType == 367326199) then
	    	iNumToPlace = iNumToPlace/1.9;
	    	print("RESOURCECLASS_LEY_LINE eResourceClassType:",self.iFrequency[row.ResourceIndex]);
	    	print("RESOURCECLASS_LEY_LINE iFrequency:",self.iFrequency[row.ResourceIndex]);
	    	print("RESOURCECLASS_LEY_LINE iOccurencesPerFrequency:",self.iOccurencesPerFrequency);
	    	print("RESOURCECLASS_LEY_LINE iNumToPlace:",iNumToPlace);
	    end
	...
	for x = 0, iW - 1 do
		for y = 0, iH - 1 do
			...
			if (ResourceBuilder.CanHaveResource(pPlot, self.eResourceType[iResourceIndex])) then
				...
				row.MapIndex = i;
				row.Score = 750;
				row.Score = row.Score / ((ResourceBuilder.GetAdjacentResourceCount(pPlot) + 1) * 100);--1.1
				row.Score = row.Score + TerrainBuilder.GetRandomNumber(100, "Resource Placement Score Adjust");
