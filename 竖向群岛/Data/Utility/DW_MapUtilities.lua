...
print("DetermineYShift start");
	...
	local group_radius = math.floor(g_iH / 10);
	-- Measure the groups.
...
function TPT_AddGoodies(iW, iH)
	local NO_PLAYER = -1;
	print("-------------------------------");
	print("Map Generation - Adding Goodies TPT");
...
--		print("改良？？")
		local improvementID = improvement.RowId - 1;
		if(improvement.Goody and not (improvement.TilesPerGoody == nil)) then
--			print("村庄类型:", improvement.ImprovementType)
			if improvement.ImprovementType == "IMPROVEMENT_GOODY_HUT" then		-- 号码菌:固定村庄密度
				improvement.TilesPerGoody = 60
				improvement.GoodyRange = 5
--				print("修改部落村庄参数")
			end
			...
			for k = 0, 2 do
				for x = 0, iW - 1 do
					for y = 0, iH - 1 do
						...
						local bGoody = CanPlaceGoodyAt(improvement, pPlot);
						if (bGoody) then
--							print("数据：", improvement.TilesPerGoody, improvement.GoodyRange)
