...
function ApplyBaseTerrain(plotTypes, terrainTypes, iW, iH)
	for i = 0, (iW * iH) - 1, 1 do
		...
		if (terrainTypes[i] == g_TERRAIN_TYPE_SNOW) then
		    rtnValue = g_TERRAIN_TYPE_TUNDRA;
		end
		if (terrainTypes[i] == g_TERRAIN_TYPE_SNOW_HILLS) then
		    rtnValue = g_TERRAIN_TYPE_TUNDRA_HILLS;
		end
...
function TeamPVPGenerateTerrainTypes(plotTypes, iW, iH, iFlags, bNoCoastalMountains, temperature, bonus_cold_shift)
	...
	local iDesertPercent = 10;
	local iPlainsPercent = 50; 
	local fSnowLatitude  = 1 + coldShift;
	local fTundraLatitude = 0.6;
	local fGrassLatitude = 0.1; 
	local fDesertBottomLatitude = 0.1;
	local fDesertTopLatitude = 0.25;
    ...
    --fSnowLatitude=10;
    iDesertPercent=0;
	local iDesertTopPercent		= 0;
	local iDesertBottomPercent	= 0;--math.max(0, math.floor(100-iDesertPercent));
