...
function SardinianPath.GetDistance(object_1, object_2)
    local result = 0
...
function SardinianPath.CheckPlot(pPlot, playerID)
    --can pass?
...
function SardinianPath.GetPlotG(index)
...
function SardinianPath:GetAStarNode(sIndex, eIndex, parent)
    --获取单元格对象
...
function SardinianPath:GetNodeNeighbor(node, eIndex, playerID)
    local neighbors = {}
    if node then
...
function SardinianPath.GetMinFNode(openTable)
...
function SardinianPath.GetMeetNode(index, openTable)
    local meetNode = nil
...
--构建路径 (GamePlay, UI)
function SardinianPath.BuildPath(path, node, meetNode)
    local pathFromStart = {}
    local pathFromEnd = {}
    --获得从起点到相遇点的路径
    while node do
...
--扩展相邻节点 (GamePlay, UI)
function SardinianPath:ExpandNeighbors(node, openTable, closeSet, eIndex, playerID)
    --获取相邻节点
...
function SardinianPath:AStar(startX, startY, endX, endY, playerID)
    --返回的结果
    local results = {}
    --起点与终点，还有玩家ID不为nil
