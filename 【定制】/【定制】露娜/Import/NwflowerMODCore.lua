--	FILE: CIVILIZATION_LUNA_Core.lua
--  VERSION: 1
--  Author: Nwflower
...
--||=========================Print========================||--
function NwModCore:__DEBUG(...)
    if DEBUG then
        print(...);
    end
end

function NwModCore:SafeSetProperty(pObject,key,value)
    if not UI then
        pObject:SetProperty(key,value);
    end
end

-- 打印一层Table
function NwModCore:ShowTable(tTable)
    if tTable == nil then print("NwModCore:ShowTable: table is nil"); return; end
...
-- 打印到指定层Table
function NwModCore:ShowRectable(tTable, iLevel)
	local level = 0;
...
function NwModCore:CheckLeaderMatched(playerID, leaderType)
...
function NwModCore:HasBoost(techOrCivic)
...
function NwModCore:CanHaveUnit(plot, unitdef)
...
function NwModCore:FormatValue(value)
    if value == 0 then
...
--数字百分比修正 (GamePlay, UI)
function NwModCore:ModifyByPercent(num, percent)
    return self:Round(num * (1 + percent / 100))
end

--获取玩家的区域数量 (GamePlay, UI)
function NwModCore:GetPlayerDistrictCount(playerID, index)
...
function NwModCore:WeightedRandomSelection(selection_tmp, num)
    -- 处理空输入
...
--设置属性 (GamePlay, UI)
function NwModCore:SetObjectState(pObject, sPropertyName, value)
    if (sPropertyName == nil) then return nil; end

    if (pObject == nil) then
        print("NwModCore SET OBJECT STATE: ERROR: OBJECT IS NIL!");
        return nil;
    end
...
function NwModCore:tableRandom(num)
    if UI ~= nil then
...
function NwModCore:DamageUnit(unit, damage)
