...
function RichNSBalance()
		...
		for i = 1, PlayerManager.GetAliveMajorsCount() do
			if (PlayerConfigurations[tempMajorList[i]]:GetLeaderTypeName() ~= "LEADER_SPECTATOR" and PlayerConfigurations[tempMajorList[i]]:GetHandicapTypeID() ~= 2021024770) then
				...
				if (pStartPlot_i ~= nil and pStartPlot_i:IsWater() == false) then
					...
					print('正在为出生在',TerrainType,'的',i,'号玩家奢侈品补偿')
					-- 先看一下玩家大陆的奢侈
					local flag = false;
	...
	for i = 0, g_iH - 1 do
		for j = 0, g_iW - 1 do
			...
			if CanHaveLeyLine and (pPlot:GetTerrainType() == 0 or pPlot:GetTerrainType() == 3) and pPlot:GetResourceType() == -1 and TerrainBuilder.GetRandomNumber(400, "Resource Placement Score Adjust") < RichNum * RichNum then
